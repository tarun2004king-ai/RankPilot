...
const seoModel = process.env.GEMINI_MODEL || "gemini-3.6-flash";
...
export async function analyzeSeoData(scrapedData){
    try{
        ...
        console.error("Gemini SEO Analysis Error", err.message);
        return{
            success: false,
            error: err.message || "Gemini SEO Analysis failed"
        }
    }
}
