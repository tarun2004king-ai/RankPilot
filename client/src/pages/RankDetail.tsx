...
export default function RankDetail() {
    ...
    const fetchTracking = async () => {
        try{
            ...
            if(res.data.success){
                const tracking = res.data.keyword as TrackingData;

                if(tracking.status === "checking"){
                    setTimeout(fetchTracking, 3000); // Poll every 3 seconds if still checking
        ...
        try{
            ...
            const pollInterval = setInterval(async () => {
                try{
                    ...
                    const updatedTracking = check.data.keyword as TrackingData | undefined;
                    if(check.data.success && updatedTracking && updatedTracking.status !== "checking"){
