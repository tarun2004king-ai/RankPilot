...
export default function RankTracker() {
    ...
    const fetchKeywords = async () => {
        try{
            ...
            console.error("Fetch Keywords Error", err.message);
        }
        setLoading(false);
    };
            ...
            if(res.data.success){
                ...
                const pollInterval = setInterval(async ()=>{
                    try{
                        ...
                        const tracking = check.data.keyword;
                        if (!tracking) {
                            clearInterval(pollInterval);
                            console.error("Rank check returned no tracking record.");
                            return;
                        }
                                    ...
                                    <div className="flex flex-col lg:flex-row lg:items-center gap-4">
                                        ...
                                        {kw.status === "completed" && (
                                            <div className="hidden md:flex items-center gap-5">
                                                ...
                                                <div className="text-center">
                                                    <p className="text-sm font-bold text-accent">{(kw as KeywordItem & { competitors?: unknown[] }).competitors?.length || 0}</p>
